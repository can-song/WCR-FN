...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fnorm(self.bn_reduce.forward(out))
        out = self.conv_sk.forward(out)
        # out = self.relu(self.bn.forward(out))
        out = self.fnorm(self.bn.forward(out))
...
class SkResNeXt(nn.Module):
    def __init__(
        ...
        tch = 64
        self.output_size = tch
        self.stages = [tch, tch * self.expansion, tch*2 *
                       self.expansion, tch*4 * self.expansion]

        self.conv_1_3x3 = nn.Conv2d(3, tch, 3, 1, 1, bias=False)
        self.bn_1 = nn.BatchNorm2d(tch)
